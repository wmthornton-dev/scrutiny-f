...
int main(void) {
    ...
    if (status != STATUS_SUCCESS) {
        ...
        fprintf(stderr, "radio_set_uplink failed: %d\n", status);

        return 1;

    }

    if (radio_get_frequency_khz() != UPLINK_FREQUENCY_KHZ) {

        fprintf(stderr, "Failed to set uplink frequency. Current: %u\n", radio_get_frequency_khz());

        return 1;

    }

    printf("Radio set to uplink frequency (%u kHz).\n", UPLINK_FREQUENCY_KHZ);
    ...
    if (memcmp(&original_packet, &g_decrypted_packet, sizeof(TelemetryPacket)) == 0) {
        ...
        // Optional: Print out the differences for debugging
